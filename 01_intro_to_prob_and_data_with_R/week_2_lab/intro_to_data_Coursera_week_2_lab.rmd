...
ggplot(data = sfo_feb_flights, aes(x = arr_delay)) + 
         geom_histogram(binwidth = 15)
sfo_feb_flights %>% 
  summarize(max_ad = max(arr_delay), on_time_prop = sum(arr_delay <= 0)/n())
```
...
sfo_feb_flights %>% group_by(carrier) %>%
  summarize(median_ad = median(arr_delay), IQR_ad = IQR(arr_delay))
```
...
nycflights %>% group_by(origin) %>% summarize(prop_on_time = sum(dep_type == "on time") / n()) %>% 
...
nycflights %>% group_by(tailnum) %>% select(tailnum, avg_speed) %>%
...
nycflights %>% group_by(dep_type) %>% summarize(prop_arr_on_time = sum(arr_type == "on time")/n())
```