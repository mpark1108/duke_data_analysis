...
```{r outliers}
# quick look at outliers boxplot and values for numerical vectors only
nc |>
...
# By default the function reports an interval for (μnonsmoker−μsmoker). 
# We can easily change this order by using the order argument:
...
nc |> 
  dplyr::summarize(min_age = min(mage, na.rm = TRUE),
...
First, I want to look at the response: age (mage) and explanatory variable: white/nonwhite (whitemom).

Do a quick boxplot comparison
```{r boxplot-pick-my-own}
boxplot(mage ~ whitemom, data = nc)
```

The boxplot above suggests nonwhite mothers are giving birth at an earlier age than whites. This is suggestive by
the lowered IQR and slightly lowered range. The box seems to show roughly equal variance between the two groups.
A t-test can be performed to address the following:

Null hypothesis: No difference between mean age of white and non-white mothers.

Alternate hypothesis: Mean age for non-white mothers is lower than mean age for white mothers.

A one sided t-test will be performed with an alpha level of 0.05.

```{r pick-your-own}
# type your code for the Exercise here, and Knit
t.test(mage ~ whitemom, data = nc, alternative = 'less')
```

Based on the test, there is significant difference with a p-value of 1.215e-07
with an alpha level of 0.05. Therefore the null hypothesis is rejected.

The evidence suggests that non-white mothers give birth at a younger age compared to white women.

Summary statistics:

```{r summary-stat-my-own}
nc |>
   dplyr::filter(!is.na(whitemom), !is.na(mage)) |>
   dplyr::summarise(
       n = dplyr::n(),
       min_age = min(mage),
       q1_age = quantile(mage, 0.25),
       median_age = median(mage),
       mean_age = mean(mage),
       q3_age = quantile(mage, 0.75),
       max_age = max(mage),
       sd_age = sd(mage),
       .by = whitemom
   )
...
Using inference() wrapper tool:

``` {r inference-tool}
inference(y = mage, x = whitemom, data = nc, statistic = "mean", type = "ht", 
