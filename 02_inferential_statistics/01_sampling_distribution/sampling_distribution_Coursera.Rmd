...
samp1 %>%
  summarize(x_bar = mean(area), samp_med = median(area), 
...
sample_means50 %>% summarize(x_means = mean(x_bar), means_med = median(x_bar),
                             sd_means = sd(x_bar), means_iqr = IQR(x_bar),
                             means_min = min(x_bar), means_max = max(x_bar),
                             means_q1 = quantile(x_bar, 0.25),
                             means_q3 = quantile(x_bar, 0.75))
...
ggplot(data = sample_mean15, aes(x = x_bar)) +
  geom_histogram(binwidth = 2500)

sample_mean15 %>%
  summarize(dist_mean = mean(x_bar))

ames %>%
  summarize(sigma = mean(price))
```
...
ggplot(data = sample_mean150, aes(x = x_bar)) +
  geom_histogram(binwidth = 2500)
