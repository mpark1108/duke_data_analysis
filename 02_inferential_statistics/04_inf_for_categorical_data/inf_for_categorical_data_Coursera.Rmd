...
us12 |> summarize(prop_atheist = sum(response == "atheist") / n())
...
us12_inf <- inference(y = response, data = us12, statistic = "proportion", type = "ci", method = "theoretical", success = "atheist")
us12_inf$ME
```
...
korea12 <- filter(atheism, nationality == "Korea, Rep (South)", atheism$year == "2012")
japan12 <- filter(atheism, nationality == "Japan", atheism$year == "2012")
...
spain_combined <- bind_rows(spain05, spain12)
ggplot(spain_combined, aes(x = response)) + geom_bar() + facet_wrap(~year)

success1 <- sum(spain05$response == "atheist")
success2 <- sum(spain12$response == "atheist")
trial1 <- nrow(spain05)
trial2 <- nrow(spain12)
...
result <- prop.test(x = successes, n = trials)
...
us_combined <- bind_rows(us05, us12)
ggplot(us_combined, aes(x = response)) + geom_bar() + facet_wrap(~year)

success1 <- sum(us05$response == "atheist")
success2 <- sum(us12$response == "atheist")
trial1 <- nrow(us05)
trial2 <- nrow(us12)
