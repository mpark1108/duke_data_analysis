...
```{r set-seed}
set.seed(3292022)                 # make sure to change the seed
```
...
samp_summary <- samp %>% summarize(samp_mean = mean(area),
                                   samp_med = median(area), s = sd(area),
                                   samp_iqr = IQR(area), samp_min = min(area),
                                   samp_max = max(area),
...
```
Description: From a random sample of 60 homes, the sample has a mean size of 1493.717 sqft, a median size of 1412.5 sqft, and a standard deviation of 534.8923 sqft. The interquartile range is 691 sqft. The minimum home size is 808 sqft and the maximum is 2978 sqft. The 25th percentile is 1079 sqft and the 75th percentile is 1770 sqft. Since the mean is larger than the median, the distribution could be slightly right-skewed. The spread above the third quartile (1208 sqft) is larger than the spread below the first quartile (271 sqft), which also suggests some right skew. Because of this right skew, the median is a better indicator of the typical home size, which is around 1400 sqft.
...
<li> The sample distribution must be nearly normal. </li> <-- answer
</ol>
...
<li> 95% of random samples of size 60 will yield confidence intervals that contain the true average area of houses in Ames, Iowa. </li> <-- answer
...
samp_CI
```
The CI from the sample of 60 homes seems to capture the true average size of houses in Ames.
...
47/50 = 0.94 is not exactly 95%. It's not the same because of sampling variation, but it is close.
...
```{r plot-99-perc-cis}
# type your code for the Exercise here, and Knit
z_star_99 = qnorm(0.99+(0.01/2))

ci_99 <- ci %>% mutate(
  center = (lower + upper) / 2,
  margin_95 = (upper - lower) / 2,
  margin_99 = margin_95 * (z_star_99 / z_star_95),
  lower = center - margin_99,
  upper = center + margin_99,
